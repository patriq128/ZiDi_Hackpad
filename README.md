...
The `Firmware` folder contains everything you need to run the firmware, including a `conf.json` file with some example modes.

The `production` folder contains `firmware.uf2`, which is the CircuitPython firmware compiled and packaged as a UF2 file, ready to be flashed onto the device.
